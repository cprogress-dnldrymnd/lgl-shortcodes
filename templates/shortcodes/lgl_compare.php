...
<script type="text/javascript">
    ...
    /**
     * Dynamically calculates the height of the user-defined offset selector (e.g., sticky site header)
     * and maps it to a CSS variable to ensure the comparison table headers don't scroll beneath it.
     * Binds to scroll and resize to catch animated or shrinking headers.
     * * @return {void}
     */
    function updateTableStickyOffset() {
        if (!lgl_ajax_obj.sticky_selector) return;

        const offsetElement = document.querySelector(lgl_ajax_obj.sticky_selector);
        if (offsetElement) {
            // Include getBoundingClientRect to factor in fractional scaling
            const currentOffset = offsetElement.getBoundingClientRect().height;
            document.documentElement.style.setProperty('--lgl-sticky-offset', currentOffset + 'px');
        }
    }

    // Bind calculation listeners
    window.addEventListener('resize', updateTableStickyOffset);
    window.addEventListener('scroll', updateTableStickyOffset, {
        passive: true
    });

    // Execute immediately on payload load
