 ...
 * Logged out → "Sign In"  |  "Register"  — each deep-links to the correct tab
 *              on the [lgl_my_account] page via ?lgl_auth=login / ?lgl_auth=register
 *
...
<?php else :
    ...
    // Build deep-link URLs that auto-open the correct tab on the account page
    $signin_url   = add_query_arg( 'lgl_auth', 'login',    $account_url );
    $register_url = add_query_arg( 'lgl_auth', 'register', $account_url );

    ?>

    <div class="lgl-mini-account lgl-mini-account--logged-out">

        <a class="lgl-mini-account__signin" href="<?php echo esc_url( $signin_url ); ?>">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/>
                <circle cx="12" cy="7" r="4"/>
            </svg>
            <?php esc_html_e( 'Sign In', 'lgl-shortcodes' ); ?>
        </a>

        <span class="lgl-mini-account__divider" aria-hidden="true">|</span>

        <a class="lgl-mini-account__register" href="<?php echo esc_url( $register_url ); ?>">
